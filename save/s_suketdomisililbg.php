...
$nosur = $kode_klasi.$slash.$no.$slash.$blnr.$slash.$thn;
        if(isset($_POST['cetak'])){
            ...
            $isi = $nosur.'Lembaga:'.$nmlbg.'Pimpinan:'.$nmket;
            $nama_qr =md5($isi);
...
mysqli_query($con, "INSERT INTO t_buatsendiri (nik, nama, kode_surat, kode_jenis, no_surat, nmsurat, tgl, status) VALUES ('$idstf','$nmket','$kode','$kodejenis','$nosur','$nmsurat','".date('Y-m-d')."','onprocess')");
...
mysqli_query($con, "INSERT INTO t_detailsurat (kode, kodejenis, nmsurat, no, nik, nama, detail, tanggal, ttd, qrcode, id_ptg) values('$kode','$kodejenis','$nmsurat','$nosur','-','$nmket', '$detail','".date('Y-m-d')."','$kades','".$nama_qr.".png','$idstf')");
